...
module.exports = {
  ...
  ByJsonSaveArticle: (Notebooklist, folder_notebook) => {
    ...
    Notebooklist.forEach(element => {
      db_promise.query(sql_str, [element.Notebookid, element.authorid, element.title, element.createtime, element.updatetime, element.content])
    });
    ...
    return db_promise.query(sql_str, [])
      ...
      .then(data => {
        ...
        if (folderid == -1 || folderid == -2) {
          ...
          sql_str = "insert into folder_notebook values(?,?);"
          return db_promise.query(sql_str, [folderid, notebookid]).then(data => {
  ...
  Output: function () {
    return db_promise.query("select * from Notebooklist", [])
      .then(
        data => {
          return db_promise.query("select * from folder_notebook", []).then(data2 => {
